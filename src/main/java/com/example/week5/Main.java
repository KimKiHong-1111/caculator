...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println();
        //가격이 16,200원 이하인 책의 목록 조회
        bookList.stream()
                .filter(book -> book.getPrice()<=16200)
                .forEach(g-> System.out.println("가격이 16200원 이하인 책의 제목: " + g.getBookName()));

        //책 제목에 "경제"라는 용어가 들어간 책의 목록 조회
        bookList.stream()
                .filter(book -> book.getBookName().contains("경제"))
                .forEach(h-> System.out.println("책 제목에 \"경제\" 라는 용어가 들어간 책의 목록 조회 : " + h.getBookName()));
        //가격이 가장 비싼 책의 가격을 조회
